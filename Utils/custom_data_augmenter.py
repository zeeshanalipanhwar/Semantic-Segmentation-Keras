...
def rotate_segmentation_data(images, masks, percent=1):
    ...
    for idx in range(0, num_of_images, step=num_of_images*percent):
...
def fliped_segmentation_data_horizontally(images, masks, percent=1):
    num_of_images = images.shape[0]
    # include the origional instances to the final list of augmented data
...
def fliped_segmentation_data_vertically(images, masks, percent=1):
    num_of_images = images.shape[0]
    # include the origional instances to the final list of augmented data
...
def augment_segmentation_data(images, masks, rotate=False, flip_horizontal=False, flip_vertical=False,
    ...
    if flip_horizontal:
        augmented_images, augmented_masks = fliped_segmentation_data_horizontally(augmented_images, augmented_masks, flip_horizontal)
    if flip_vertical:
